...
describe CodeUnpacker do
  ...
  let(:file) { StringIO.new(packed_data) }
  let(:unpacker) { CodeUnpacker.new(file) }

  context "with 4 literals" do
    let(:packed_data) { "\x2F\x00ABCD\x00\x00\x00" }
    let(:unpacked_data) { "ABCD" }

    it "unpacks correctly" do
      expect(unpacker.unpack).to eq(unpacked_data)
    end
  end

  context "with 18 literals" do
    let(:packed_data) { "\xFF\xFFABCDEFGHIJKLMNO\x0B\x00PQR\x00\x00\x00" }
    let(:unpacked_data) { "ABCDEFGHIJKLMNOPQR" }
